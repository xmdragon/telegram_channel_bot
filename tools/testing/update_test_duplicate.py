...
async def update_test_duplicate():
    ...
    try:
        ...
        # 更新原始消息
        original_msg = redis_store.get_message(test_channel, original_msg_id)
        if original_msg:
            original_msg.update({
            ...
            redis_store.save_message(test_channel, original_msg_id, original_msg)
            print(f"✅ 原始消息已更新: {test_channel}:{original_msg_id}")
        else:
            print(f"❌ 找不到原始消息: {test_channel}:{original_msg_id}")
        
        # 更新重复消息
        duplicate_msg = redis_store.get_message(test_channel, duplicate_msg_id)
        if duplicate_msg:
            ...
            redis_store.save_message(test_channel, duplicate_msg_id, duplicate_msg)
        ...
        # 验证更新
        print("\n验证更新结果...")
        orig_updated = redis_store.get_message(test_channel, original_msg_id)
        dup_updated = redis_store.get_message(test_channel, duplicate_msg_id)
