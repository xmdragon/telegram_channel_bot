...
class SemanticTailFilter:
    ...
    def calculate_hybrid_scores(self, tail: str, full_content: str, position: int, lines: List[str]) -> Dict[str, float]:
        """
        计算多维度综合得分（整合自hybrid_tail_filter）
        ...
        # 1. 语义得分（0-1）
        scores['semantic'] = self.calculate_semantic_score(tail, full_content)
        
        # 2. 结构得分（0-1）- 基于已有逻辑
        structural_score = 0.0
        
        # 检查分隔符
        if re.search(r'^[-=*#_~—]{3,}$', tail, re.MULTILINE):
            structural_score += 0.3
        
        # 检查emoji分隔
        if re.search(r'^\s*[📣🔔😍👌💬🔗📢]{3,}\s*$', tail, re.MULTILINE):
            structural_score += 0.2
        
        # 检查链接密度
        link_count = len(re.findall(r'https?://|t\.me/', tail))
        if link_count > 0:
            link_density = link_count / len(tail.split('\n'))
            structural_score += min(0.3, link_density * 0.5)
        
        # 检查用户名密度
        username_count = len(re.findall(r'@\w+', tail))
        if username_count > 0:
            username_density = username_count / len(tail.split('\n'))
            structural_score += min(0.2, username_density * 0.4)
        
        scores['structural'] = min(1.0, structural_score)
        
        # 3. 位置得分（0-1）- 越靠后越可能是尾部
        # 使用非线性函数，让靠后的位置得分更高
        relative_position = (len(lines) - position) / min(15, len(lines))
        scores['position'] = min(1.0, relative_position ** 0.7)
        
        # 4. 相关性得分（0-1）- 与正文相关性越低越可能是尾部
        relevance = self.calculate_relevance(tail, full_content)
    ...
    def calculate_weighted_score(self, scores: Dict[str, float]) -> float:
        """
        计算加权综合得分（整合自hybrid_tail_filter）
    ...
    def filter_message_hybrid(self, content: str, has_media: bool = False) -> tuple:
        """
        使用混合多维度算法进行尾部过滤（整合自hybrid_tail_filter）
        ...
        best_tail = None
        threshold = 0.6  # 综合得分阈值
        max_tail_ratio = 0.5  # 尾部最大占比
        min_tail_length = 20  # 最小尾部长度
        ...
        for i in range(scan_start, scan_end, -1):
            ...
            # 跳过太短的内容
            if len(tail_candidate) < min_tail_length:
                continue
            
            # 快速预检：如果没有任何推广特征，跳过
            if not any(indicator in tail_candidate for indicator in ['@', 't.me/', 'http', '订阅', '加入', '🔔', '📣']):
                continue
            
            # 计算四个维度的得分
            scores = self.calculate_hybrid_scores(tail_candidate, content, i, lines)
        ...
        if best_tail:
            ...
            logger.info(f"混合算法过滤成功，得分: {best_score:.3f}, "
            ...
            return clean_content, True, best_tail, analysis
