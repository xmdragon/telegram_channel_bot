...
1. **先读CLAUDE.md** - 每次都读，不是"记得"
   ```bash
   cat CLAUDE.md | grep -A 5 "开发规范"
   ```

2. **API相关代码前必须执行**：
   ```bash
   # 检查路由定义
   cat app/core/route_config.py | grep "要实现的功能"
   cat static/assets/js/config/api-endpoints.js | grep "相关端点"
...
### 自我约束承诺：
**我承诺每次修改代码都会先执行上述检查。如果违反，请直接说"重写"，我会无条件重新按规范实现。**
